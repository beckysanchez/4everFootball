...
document.addEventListener("DOMContentLoaded", () => {
 ...
 // ========== VALIDACIÓN BÁSICA DE PAÍS Y NACIONALIDAD (sin API) ==========
const countryInput     = document.getElementById("country");
const nationalityInput = document.getElementById("nationality");

function validarCampoTexto(el) {
  const val = el.value.trim();
  
  // Valida: no vacío, solo letras/espacios, mínimo 3 caracteres
  const ok = val.length >= 3 && /^[a-zA-ZÁÉÍÓÚÑáéíóúñ\s]+$/.test(val);

  el.classList.toggle("is-valid", ok);
  el.classList.toggle("is-invalid", !ok);
  el.setCustomValidity(ok ? "" : "Ingresa un valor válido.");
}

// Eventos para validar mientras escribe y al salir del campo
[countryInput, nationalityInput].forEach(input => {
